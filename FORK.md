...
| [#3486](https://github.com/wavetermdev/waveterm/pull/3486) | OSC 8 hyperlinks open again — xterm's built-in handler calls `window.open()` with no URL, which Wave's window-open policy denies, so clicking one did nothing               | Opens on the same modifier-click as the fork's own detected links (⌘ / Ctrl) rather than a bare click, and feeds the same hover tooltip + "open link" context menu; both gestures now share `isLinkActivationEvent`. Only http(s) links reach the handler (xterm's `allowNonHttpProtocols` stays off), so `file://` hyperlinks from `ls --hyperlink` are still inert. |
| [#3498](https://github.com/wavetermdev/waveterm/pull/3498) | PowerShell env vars are emitted as `${env:NAME}`, so Windows-standard names like `ProgramFiles(x86)` survive the token swap                                                 | Upstream's replacement check rejects only ``} ` =`` and the empty name; the fork also rejects control characters, since a newline in a name would otherwise inject a line into the generated script. Covered by `pkg/util/shellutil/tokenswap_test.go`.                                                                                                               |
...
The first ten rows shipped in **v0.14.5-pj.21**. A second sweep (2026-09-15) picked up #3486 and
#3498 — everything else opened upstream since is dependabot, already-fixed fork-side
([#3485](https://github.com/wavetermdev/waveterm/pull/3485), per-job reconnect deadlines), or
[#3507](https://github.com/wavetermdev/waveterm/pull/3507), whose GPU metrics rewrite the two
files the fork's system monitor owns and needs a hand-merge rather than a cherry-pick.
