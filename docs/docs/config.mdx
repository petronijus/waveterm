...
| term:trimtrailingwhitespace          | bool     | trim trailing whitespace from every line when copying a terminal selection (default true)                                                                                                                                                                     |
| term:copyunwrap                      | bool     | when copying, re-join lines that TUI programs (Claude Code, tmux, less, ...) hard-wrapped at the terminal width, so URLs and paragraphs paste as one line (default true; "Copy Raw" in the context menu bypasses it)                                          |
| term:aliases                         | map      | text expansions for terminal input, keyed by the trigger word (`{"AtoRun": "docker compose up -d"}`); typing the word then a space or tab replaces it, in the shell and inside a TUI alike (never presses Enter)                                              |
...
| window:disablegraphite               | bool     | set to opt out of Chromium's Skia Graphite raster backend (`--disable-features=SkiaGraphite`) but keep hardware acceleration; for GPU-process crashes inside Graphite, e.g. a macOS beta with a broken Metal shader compiler (requires app restart)           |
